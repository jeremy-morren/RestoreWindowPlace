...
public class MonitorLayoutTests
{
    ...
    [Fact]
    public void Which_monitor_is_primary_changes_the_key()
    {
    ...
    [Fact]
    public void Renaming_a_display_device_does_not_change_the_key()
    {
    ...
    [Fact]
    public void The_key_does_not_depend_on_the_current_culture()
    {
    ...
    public void IsVisible_reports_whether_a_window_would_land_somewhere_reachable(
    ...
    [Fact]
    public void The_attached_monitors_can_be_read_from_the_operating_system()
    {
        Desktop.Require();
    ...
    [Fact]
    public void A_real_multi_monitor_arrangement_is_coherent()
    {
        Desktop.Require();
        ...
        // Windows lays monitors out side by side on the virtual desktop; two that overlapped
        // would mean the bounds are being read in the wrong DPI context, or not at all.
        ...
        // Losing a monitor has to change the key — that is the entire point of keying
        // placements by arrangement.
        new MonitorLayout(layout.Monitors.Take(1)).Key.Should().NotBe(layout.Key);
    }

    [Fact]
    public void Reading_the_attached_monitors_twice_gives_the_same_key() =>
        MonitorLayout.GetCurrent().Key.Should().Be(MonitorLayout.GetCurrent().Key);

    [Fact]
    public void The_key_does_not_depend_on_the_calling_thread_s_dpi_awareness()
    {
        Desktop.Require();
