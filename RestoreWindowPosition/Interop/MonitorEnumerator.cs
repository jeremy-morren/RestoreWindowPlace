...
internal static class MonitorEnumerator
{
    ...
    /// <returns>
    /// The monitors, in whatever order the operating system reports them. Empty if the
    /// desktop cannot be enumerated, for instance from a service in session 0.
    /// </returns>
    public static List<MonitorInfo> GetMonitors()
    {
        ...
        {
            if (restore != IntPtr.Zero) NativeMethods.SetThreadDpiAwarenessContext(restore);
        }
    }
    ...
    private static List<MonitorInfo> Enumerate()
    {
        ...
        try
        {
            ...
            unsafe
            {
                NativeMethods.EnumDisplayMonitors(IntPtr.Zero, IntPtr.Zero, &OnMonitor, GCHandle.ToIntPtr(handle));
            }
#else
            NativeMethods.EnumDisplayMonitors(IntPtr.Zero, IntPtr.Zero, OnMonitor, GCHandle.ToIntPtr(handle));
#endif
    ...
    /// <remarks>
    /// <para>
    /// Windows reports monitor rectangles in the DPI context of the calling thread: a process
    /// that is not DPI aware sees a 3840x2160 monitor at 150% as 2560x1440. A WPF application
    /// becomes DPI aware partway through startup, so the same machine reports one arrangement
    /// from the <c>App</c> constructor and a different one a moment later — and a key derived
    /// from it would never match itself across a run.
    /// </para>
    /// <para>
    ...
    private static IntPtr EnterPerMonitorAwareness()
    {
        ...
        try
        {
            // V2 arrived in Windows 10 1703 and V1 in 1607; the call simply returns null for
            // a context this build does not know.
        ...
        {
            // Windows 8.1 and earlier. Fall back to the caller's context, which on those
            // versions cannot change partway through a process anyway.
    ...
    private static unsafe bool TryDescribe(IntPtr hMonitor, out MonitorInfo monitor)
    {
